...
filters.lmt_time = {
	decode: function(value) {
		if(value) {
			return value.slice(8).toString();
		}
		return value;
	}
}

filters.lmt_message_id = {
	decode: function(value) {
		if(value) {
			return value.slice(3).toString();
		}
		return value;
	}
}
 ...
 * 
 * # 8 prefix
   lmt_created_ts                  => [undef, 0x1500, 'OCT', 1, -14],
   lmt_subscriber_id               => [undef, 0x1501, 'OCT', 1, 20],
   lmt_response_till               => [undef, 0x1502, 'OCT', 1, -14],
   lmt_tariff_class                => [undef, 0x1503, 'INT', 2, 0],
   lmt_more_messages_to_send       => [undef, 0x1504, 'INT', 1, 0],
   lmt_service_desc                => [undef, 0x1505, 'OCT', 1, 40],
   lmt_message_id                  => [undef, 0x1506, 'OCT', 1, 65],
   lmt_report_type                 => [undef, 0x1508, 'INT', 1, 0],
   lmt_transaction_id              => [undef, 0x1509, 'INT', 4, 0],
   lmt_addr                        => [undef, 0x1510, 'OCT', 1, 20],
   lmt_addr_ton     
 * 
 */
...
var commands = {
	...
	lmt_report: {
		id: 0x00010000,
		params: {
			lmt_created_ts: { type: types.string, filter: filters.lmt_time },
			lmt_report_type: { type: types.int8 },
			lmt_message_id: { type: types.string, filter: filters.lmt_message_id },
			lmt_subscriber_id: { type: types.string }
		}
	},
	lmt_report_resp: {
		id: 0x80010000
